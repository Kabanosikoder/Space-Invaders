...
MAX_ALIENS = 20  # This is a hard limit on aliens spawning due to pyturtle slowing down once too many are present
...
def create_laser():
    ...
    if current_time - last_cannon_shot_time >= CANNON_FIRE_RATE and DIFFICULTY_LEVEL != 0:
        laser = turtle.Turtle()
        laser.penup()
        laser.color("red")
        laser.shape("square")
...
window.onkey(lambda: None, "Left")
window.onkey(lambda: None, "Right")
...
def difficulty_menu():
    ...
    logo.write("SPACE INVADERS", align="center", font=("Courier", 40, "bold"))
...
# function nesting, rarely used this
    def set_difficulty(level):
        global ALIEN_SPEED, ALIEN_SPAWN_INTERVAL, LASER_SPEED, DIFFICULTY_LEVEL, CANNON_FIRE_RATE, ALIEN_FIRE_RATE, diff
        if level == 1:
            diff = "easy"
            ALIEN_SPEED = 0.25
